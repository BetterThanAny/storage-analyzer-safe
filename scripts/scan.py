...
def du_size(path, timeout=120):
    ...
    denied = r.returncode != 0 or "Permission denied" in (r.stderr or "")
    return (int(m.group(1)) if m else None), denied
...
def du_children(path, min_kb=51200, limit=40):
    ...
    for name in entries:
        ...
        if os.path.islink(child):
            continue
        kb, denied = du_size(child, timeout=120)
        if kb is None:
            if denied:
...
MAC_TARGETS = [
    ...
    ("system_library", "/Library", 102400),
    ("users_shared", "/Users/Shared", 51200),
...
def dev_caches_macos():
    ...
    for p in MAC_DEV_CACHE_PATHS:
        ...
        if not os.path.isdir(path):
            continue
        kb, denied = du_size(path, timeout=180)
        if kb is None:
            continue
        if kb < 51200:
            continue
...
def system_info_macos():
    ...
    name = re.search(r"Volume Name:\s*(.+)", dinfo)
    info["disk_name"] = name.group(1).strip() if name else "Startup Disk"
    info["disks"] = mounted_disks_macos(info["disk_name"], total, used, free)
    return info


def mounted_disks_macos(root_name, root_total, root_used, root_free):
    disks = [{"name": root_name, "total": root_total, "used": root_used, "free": root_free}]
    seen_mounts = {"/"}
    for line in run(["df", "-kP"]).splitlines()[1:]:
        parts = line.split(None, 5)
        if len(parts) < 6:
            continue
        _, total, used, free, _, mount = parts
        if mount in seen_mounts or not mount.startswith("/Volumes/"):
            continue
        seen_mounts.add(mount)
        try:
            disks.append({"name": os.path.basename(mount) or mount,
                          "total": human(int(total)),
                          "used": human(int(used)),
                          "free": human(int(free))})
        except ValueError:
            continue
...
def dir_size_bytes(path):
    ...
    try:
        with os.scandir(path) as it:
            for e in it:
                try:
                    if e.is_symlink() or entry_is_reparse(e):
                        continue
